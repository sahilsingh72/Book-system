                ...
                <div class="card-body">
                    ...
                    <form action="{{ route('admin.admins.update', $admin->id) }}" method="POST">
                        ...
                        <div class="form-row">
                            ...
                            <div class="form-group col-md-6 col-sm-6">
                                <label for="assign_under">Assign Under {{$admin->hasRole($admin->name)}}</label>
                                <input type="text" class="form-control" id="assign_under" name="assign_under"  value="{{ $admin->name }}" disabled>
                                {{-- <select name="assign_under" id="assign_under" class="form-control " required disabled>
                                    @foreach ($admins as $admin)
                                        <option value="{{ $admin->name }}" {{ $admin->hasRole($admin->name) ? 'selected' : '' }}>{{ $admin->name }}</option>
                                    @endforeach
                                </select> --}}
                            </div>
                            <div class="form-group col-md-6 col-sm-6">
